...
<form id="workExperienceForm" action="<?php echo e(route('work_experience_store')); ?>" method="POST">
  <?php echo csrf_field(); ?>
  ...
  <main class="max-w-full md:max-w-5xl mx-auto bg-white border border-gray-300 mobile-padding p-4 md:p-6 rounded-[10px] shadow-md overflow-x-auto"
        x-data="{
          entries: (() => {
            let data = <?php echo e(old('entries') ? json_encode(old('entries')) : ($workEntries->isEmpty() ? json_encode([[ 'start_date' => '', 'end_date' => '', 'position' => '', 'office' => '', 'supervisor' => '', 'agency' => '', 'accomplishments' => [''], 'duties' => [''], 'isDisplayed' => true, ]]) : $workEntries->toJson())); ?>;
            ...
            <div class="mobile-form-element">
              ...
              <div class="flex flex-col sm:flex-row mobile-duration gap-2">
                ...
                <div class="flex flex-col w-full">
                  <input :class="entry.present ? 'text-gray-400' : ''" :readonly="!entry.isDisplayed || entry.present" :required="!entry.present" :name="'entries[' + index + '][end_date]'" type="date" class="mobile-input w-full border rounded px-3 py-2" x-model="entry.end_date">
  ...
  <div class="mobile-footer max-w-full md:max-w-5xl mx-auto mt-6 flex flex-col md:flex-row gap-4 justify-between items-center">
    <button type="button" onclick="window.location.href='<?php echo e(route('dashboard_user')); ?>'" class="mobile-button use-loader w-full md:w-auto bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400 flex items-center gap-1 justify-center">
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('after_action') === 'download'): ?>
<script>
  window.onload = function () {
    window.location.href = "<?php echo e(route('export.wes')); ?>";
  };
</script>
<?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
</html><?php /**PATH C:\xampp\htdocs\PDS-main\resources\views/dashboard_user/work_exp.blade.php ENDPATH**/ ?>