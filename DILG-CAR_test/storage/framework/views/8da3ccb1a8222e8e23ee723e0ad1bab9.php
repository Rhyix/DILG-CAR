<?php $__env->startSection('title','Work Experience Sheet'); ?>
...
<form id="workExperienceForm" action="<?php echo e(route('work_experience_store')); ?>" method="POST">
  ...
  <div class="max-w-full md:max-w-5xl mx-auto mt-6 flex flex-col md:flex-row gap-4 justify-between items-center">
    <button type="button" onclick="window.location.href='<?php echo e(route('display_c4')); ?>'" class="use-loader w-full md:w-auto bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400 flex items-center gap-1 justify-center">
      ← Previous
    </button>
    ...
    <div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto justify-end">
      ...
      <button type="button" onclick="window.location.href='<?php echo e(route('display_c5')); ?>'" class="use-loader bg-yellow-500 text-gray-900 px-4 py-2 rounded hover:bg-yellow-600 flex items-center gap-1 justify-center">
        Next: Upload PDF →
      </button>
    </div>
  </div>
...
<?php echo $__env->make('layout.pds_layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\PDS-main\resources\views/pds/wes.blade.php ENDPATH**/ ?>