...
function storeInSession(key, value) {
    try {
        ...
        // Ignore browsers or modes that block sessionStorage.
    }
}

document.addEventListener('DOMContentLoaded', () => {
    const configElement = document.getElementById('landingDocumentConfig');
    const documentMeta = JSON.parse(configElement?.dataset.documentMeta ?? '{}');
    const requiredDocsByTrack = JSON.parse(configElement?.dataset.requiredDocs ?? '{}');
    ...
    function normalizeVacancyTrack(vacancyType) {
        const value = String(vacancyType || '').trim().toLowerCase();

        return value === 'cos' || value === 'contract of service' ? 'COS' : 'Plantilla';
    }

    function requiredDocumentsForTrack(track) {
        const required = new Set(requiredDocsByTrack[track] || []);

        return Object.keys(documentMeta)
            .filter((documentType) => required.has(documentType))
            .map((documentType) => documentMeta[documentType]);
    }
    ...
    function openModal(job) {
        storeInSession('selectedJobId', String(job.vacancy_id ?? ''));
        storeInSession('selectedJobTitle', String(job.position_title ?? ''));
        ...
        modalEligibility.textContent = String(job.qualification_eligibility || 'N/A')
        ...
        const labels = requiredDocumentsForTrack(normalizedTrack);
        ...
        labels.forEach((label) => {
            ...
            item.innerHTML = `${CHECK_ICON}<span>${label} <span class="text-red-600">*</span></span>`;
    ...
    document.querySelectorAll('.vacancy-card').forEach((card) => {
        card.addEventListener('click', () => {
            ...
            openModal(JSON.parse(window.atob(encodedData)));
        });
    });
