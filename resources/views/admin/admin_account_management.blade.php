...
@section('content')
    ...
    <main class="w-full h-[calc(100vh-6rem)] flex flex-col space-y-4 overflow-hidden">
        ...
        <section class="flex-none flex items-center space-x-4 max-w-full">
            <h1
                class="flex items-center gap-3 w-full border-b border-[#0D2B70] text-white text-4xl font-montserrat tracking-wide select-none">
                        ...
                        @foreach ($admins as $admin)
                                        <tr class="text-[#0D2B70] select-none hover:bg-blue-50 transition-colors duration-200">
                                            ...
                                            <td class="py-4 px-6 text-center w-[15%]">
                                                <span
                                                    class="px-3 py-1 rounded-full border-2
                                            ...
                                            <td class="py-4 px-6 text-center w-[20%]">
                                                <div class="flex justify-center items-center gap-3">
                                                    <form method="POST"
                                                        ...
                                                        @csrf
                                                        <button type="submit"
                                                            title="{{ $admin->is_active ? 'Deactivate Admin' : 'Activate Admin' }}" class="use-loader 
                                                            {{ $admin->is_active
                            ? 'text-[#C5292F]'
                            : 'text-green-600 border border-green-600 hover:bg-green-600' }} 
                                                            ...
                                                            <i data-feather="{{ $admin->is_active ? 'user-x' : 'user-check' }}"
                                                                class="w-5 h-5"></i>
                                                        </button>
                                                    </form>
