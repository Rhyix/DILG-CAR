...
<tr class="text-[#0D2B70] select-none hover:bg-blue-50 transition-colors duration-200">
  ...
  <td class="w-[15%] py-4 px-6">₱{{ number_format($vacancy->monthly_salary, 2) }}</td>
  
  <td class="w-[15%] py-4 px-6">{{ \Carbon\Carbon::parse($vacancy->closing_date)->subMinute()->format('n/j/Y g:i A') }}</td>
  
  <td class="w-[20%] py-4 px-6">{{ $vacancy->place_of_assignment }}</td>
  
  <td class="w-[10%] py-4 px-6 text-center">
